...
import './AnimatedLogoutButton.css'; // Impor CSS yang baru kita buat

// Ini adalah data state animasi dari file script.js
...
// Komponen tombol kita
// Dia akan menerima 'onClick' sebagai prop (fungsi logout asli)
const AnimatedLogoutButton = ({ onClick }) => {
  ...
  const handleClick = () => {
    if (animationState !== 'default' && animationState !== 'hover') {
      return; // Animasi sedang berjalan, jangan klik lagi
    }
    
    setClassState('clicked');
    updateButtonState('walking1');
    
    // Rantai setTimeout untuk mengontrol animasi
    setTimeout(() => {
      setClassState('clicked door-slammed');
      updateButtonState('walking2');
      
      setTimeout(() => {
        setClassState('clicked door-slammed falling');
        updateButtonState('falling1');
        
        // --- PANGGIL FUNGSI LOGOUT ASLI DI SINI ---
        // Panggil logout() TEPAT SAAT animasi jatuh dimulai
        if (onClick) {
          onClick(); 
        }
        // ------------------------------------------
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              // Reset state jika user belum pindah halaman
              setClassState('');
              updateButtonState('default');
            }, 1000); // Waktu reset
  ...
  // Salin-tempel JSX dari index.html dan konversi ke React (className, strokeWidth)
  return (
    <button 
      className={`logoutButton ${classState}`} 
      ref={buttonRef}
